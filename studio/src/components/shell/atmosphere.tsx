...
// One full sweep through every hue, not just the five named moods — a
// constant-speed (linear, not eased) rotation so it reads as a smooth
// current rather than a pulse. 70s is deliberately off the drift fields'
// own periods (52/56/64s) so the two motions never fall into lockstep.
const HUE_ROTATE_S = 70
...
@keyframes atm-hue { from { filter: hue-rotate(0deg); } to { filter: hue-rotate(360deg); } }
...
.atm-hue { animation: atm-hue ${HUE_ROTATE_S}s linear infinite; }
@media (prefers-reduced-motion: reduce) { .atm-field, .atm-hue { animation: none !important; } }
 ...
 * over about a minute and continuously sweep through every hue in the
 * spectrum together — `mood` sets where they start and how they sit relative
 * to each other, not a fixed colour they're stuck at. `intensity` (0–1)
 * scales how present the colour is — low energy reads as a quieter sky.
 * Respects prefers-reduced-motion. Sits behind everything.
 */
export function Atmosphere({ mood = 'neutral', intensity = 1 }: { mood?: Mood; intensity?: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Only the colour fields rotate through hue — a wrapper of their own so
         the ink base and the grain above never tint along with them. */}
      <div className="atm-hue" style={{ position: 'absolute', inset: 0 }}>

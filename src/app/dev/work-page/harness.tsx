...
function project(intent: string) {
  return {
    ...
    settings: { snap: true, grid: false, sizes: true }, auto_layout: true, composed_at: null,
...
function installMock(text: string, concept: boolean) {
  ...
  window.fetch = (input, init) => {
    ...
    if (path === '/api/studio/projects/demo/tree') return Promise.resolve(json({ project: project(text), tree: { nodes: [node(text, concept)], threads: [], tags: [], open_checks: [] } }))
    ...
    if (path.startsWith('/api/studio/') && init?.method && init.method !== 'GET') return Promise.resolve(json({ success: true }))
...
function Inner() {
  ...
  if (typeof window !== 'undefined') installMock(text, !!params.get('concept'))
